...
def analyze_upgrade_path(
    current_image: str, target_framework: str, target_version: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        tag_parts = tag.split("-")
        if len(tag_parts) < 4:
            return {"success": False, "error": "Invalid image tag format"}
        ...
        if current_framework == "pytorch" and target_framework.lower() == "pytorch":
            ...
            if target_major > current_major:
                compatibility_issues.append(
                    f"Major version upgrade from {current_version} to {target_version} may have breaking API changes"
                )
                upgrade_steps.append("Review PyTorch migration guides for API changes")
                upgrade_steps.append(
                    "Update model architecture and training code for compatibility"
                )

                if current_major == 1 and target_major == 2:
                    upgrade_steps.append(
                        "Consider using torch.compile() for performance improvements in PyTorch 2.x"
                    )
                    upgrade_steps.append("Update deprecated nn.Module APIs")

            if cuda_version:
                upgrade_steps.append(
                    f"Verify CUDA compatibility with target PyTorch version {target_version}"
                )
        ...
        if framework_change:
            compatibility_issues.append(
                f"Changing framework from {current_framework} to {target_framework} requires model conversion"
            )
        ...
        target_tag_components = [target_version, device_type, latest_python_version]
...
def generate_upgrade_dockerfile(
    current_image: str,
    target_image: str,
    preserve_custom_files: List[str] = [],
    ...
    if preserve_custom_files:
        dockerfile_lines.extend(
            ["# Create temporary directory for files to preserve", "RUN mkdir -p /tmp/preserve", ""]
        )

        for file_path in preserve_custom_files:
            dockerfile_lines.append(f"# Copy {file_path} to preserve")
            dockerfile_lines.append(
                f"RUN if [ -e {file_path} ]; then cp -r {file_path} /tmp/preserve/; fi"
            )

        dockerfile_lines.append("")

    # Start with the target image
    dockerfile_lines.extend(["# Start with the target image", f"FROM {target_image}", ""])
    ...
    if preserve_custom_files:
        ...
        for file_path in preserve_custom_files:
            ...
            dockerfile_lines.append(
                f"RUN if [ -e /tmp/preserve/{base_name} ]; then mkdir -p {dir_name} && cp -r /tmp/preserve/{base_name} {file_path}; fi"
            )

        dockerfile_lines.extend(
            ["", "# Clean up temporary directory", "RUN rm -rf /tmp/preserve", ""]
        )
    ...
    # Add final comments
    dockerfile_lines.extend(["# Image upgraded successfully", "WORKDIR /opt/ml"])
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            build_result = build_image(
                dockerfile_path=dockerfile_path, tag=image_tag, context_path=temp_dir
            )
...
def register_module(mcp: FastMCP) -> None:
    ...
    mcp.add_tool(
        ...
        fn=generate_upgrade_dockerfile,
    )
